...
Newton의 제2법칙은 단일입자에 대한 힘과 선운동량 사이의 관계로 다음과 같다.
$$\vec{F}_{net} = \frac{d\vec{p}}{dt} \quad \text{(단일입자)}$$
선형운동에 관한 물리량과 회전운동에 관한 물리량 사이에 서로 대응하는 관계가 있으므로 토크와 각운동량 사이에 성립하는 관계가 있을 것으로 예상하여 위 식으로부터 아래의 식을 도출할 수 있다.
$$\vec{\tau}_{net} = \frac{d\vec{\ell}}{dt}$$

> 입자에 적용하는 모든 토크들의 벡터합은 입자 각운동량의 시간변화율과 같다.
...
![150](../../Z.%20Docs/img/Pasted%20image%2020250414103817.png)![210](../../Z.%20Docs/img/Pasted%20image%2020250414103834.png)
z축에 대하여 회전하고 있는 강체의 각운동량을 구해보자.
강체의 각운동량은 각 질량요소의 각운동량의 z축 성분을 모두 더해서 구할 수 있다.
$$
\begin{align}
L_\mathcal{z} &= \sum^{n}_{i=1} \mathcal{l}_{\mathcal{i_z}} = \sum^{n}_{i=1} \Delta m_i v_i r_{\perp i} = \sum^{n}_{i=1} \Delta m_i (\omega r_{\perp i}) r_{\perp i} \\
&= \omega\ (\sum^{n}_{i=1} \Delta m_i r_{\perp i}^2)
\end{align}
$$

물리량 $\sum^{n}_{i=1} \Delta m_i r_{\perp i}^2$은 고정된 축에 대한 강체의 회전관성이 $I$이므로 다음과 같이 간략하게 표기할 수 있다.
$$L = I\omega$$

## 11-8. 각운동량의 보존

계에 작용하는 알짜 외부 토크 $\vec{\tau}_{net}$ 가 없을 때 계의 각운동량 $\vec{L}$은 일정하다.
$$\vec{L} = \text{constant}$$
- 외부에서 계에 작용하는 알짜 토크가 없다면 계의 각운동량 $\vec{L}$은 계 내부의 변화와 무관하게 일정하다.
- 만약 축 방향으로 계에 작용하는 알짜 외부 토크가 0이라면 그 축에 대한 각운동량은 계 내부의 변화에도 불구하고 변하지 않는다.

e.g. 11.06 원판 위의 바퀴벌레에서 본 각운동량 보존
`silent camera 참고`

e.g. Completely melastic collision between a ball & a rod
`silent camera 참고`
$$
\begin{cases}
L_i = m\cdot \frac L2 \cdot v \\
L_f = I_{tot} \omega = (\frac1{12}\cdot M\cdot L^2 + m(\frac{L}{2})^2)\omega
\end{cases}
$$
이 식으로부터 각운동량의 보존 법칙을 사용하면 다음과 같은 결과가 도출된다.
$$L_i = L_f = \frac{6m}{M+3m}\cdot \frac{V}{L}$$
$$
\begin{align}
\Delta K &= \frac{1}{2}I_{tot}\omega^2 = \frac12 mv^2 \\
&= ..\text{silent camera 참고}
\end{align}
$$
## 11-9. 자이로스코프의 축돌기 운동

`silent camera 및 ChatGPT 참고`

$$
\begin{align}
d\phi &= \frac{dL}{L} = \frac{\tau dt}{L} = \frac{Mgr\ dt}{I\omega} \\
&\leadsto \Omega = \frac{d\phi}{dt} = \frac{Mgr}{I\omega}
\end{align}
$$

- $\omega$가 $\Omega$보다 월등히 클 때 즉, 빠르다고 가정하면 성립
- $I$가 질량 $M$의 함수기 때문에 $\Omega$는 질량에 무관하다.
- 자전하는 자이로스코프의 축이 수평면에 대하여 기울어 있어도 성립한다. `e.g. 팽이`
...
다음의 물체들을 생각해보자.
- 책상 위에 놓은 책
- 마찰이 없는 표면을 일정한 속도로 미끄러져 가는 하키 퍽
- 천장에 달린 선풍기의 회전날개
- 직선 궤도로 등속운동하는 자전거의 바퀴
이들 모두에 대해 다음이 성립한다

1. 질량중심의 선운동량 $\vec{P}$가 일정하다.
2. 질량중심이나 임의의 점에 대한 각운동량 $\vec{L}$도 일정하다.

이럴 경우 이를 평형 상태에 있다고 하고, 다음과 같은 두 가지 조건이 성립한다.
$$\vec{P} = \text{constant}, \ \vec{L} = \text{constant}$$

(1) 정적 평형상태
$$\vec{P} = 0, \ \vec{L} = 0$$
위에 열거한 네 개의 물체 중 책상 위에 놓인 책만이 정적 평형상태에 있다.
- 안정한 평형상태: 물체가 힘을 받아 움직였다가 다시 정적 평형상태로 되돌아올 때
- 불안정한 평형상태: 작은 힘으로도 물체를 움직여 평형상태에서 벗어나게 할 수 있을 때

**무게중심**

$$\vec{r}_{cog} = \frac{\sum_i F_{g, i}\vec{r}_i}{\sum_i F_{g, i}} = \frac{\sum_i m_i g_i\vec{r}_i}{\sum_i m_i g_i}$$
- Torque on a regie body by $F_g$ in terms COG
$$\tau_{net} = \sum x_iF_{g, i} = \frac{\sum_i x_i F_{g, i}}{\sum_i F_{g, i}} = x_{cog}\cdot F_g$$
- 자유 낙하시의 중력가속도 $\vec{g}$가 물체의 모든 부분에 대해 같다면($g_i = g$) 물체의 질량중심과 무게중심은 일치한다.

## 12-2. 정적 평형의 몇 가지 보기

e.g. 12.01 수평막대 균형 잡기
`silent camera 참고`

e.g. 12.02 벽에 기대어 옪은 지지대 균형 잡기
`silent camera 참고`
...
실제로는 모든 강체(Rigid Body)들이 어느 정도는 탄성을 갖고 있다.
- Elasticity(탄성): 외력을 받아 변형된 물체가 외력이 제거되면 원래의 형태로 되돌아가려는 성질
- Stress(변형력): 단위 면적당 작용하는 내부 힘
$$\text{Stress} = \frac{F}{A}$$
	![450](https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTIjL46Kjw-4_3x7EkADopFXLx1o35ZDx-GSO1JORitHxr9PeYB)
	1. 장력: 길이가 늘어나는 경우
	2. 층밀리기 변형력: 모양이 변하는 경우
	3. 유압 변형력: 부피가 줄어드는 경우
- Strain(변형률): 물체가 변형된 정도, 원래 길이에 대한 변화율
$$\text{Strain}(\epsilon) = \frac{\Delta L}{L}$$
	![200](https://wiki1.kr/images/3/35/%EA%B0%95%EC%B2%A0%EC%9D%98_%EB%B3%80%ED%98%95%EA%B3%A1%EC%84%A0.jpg)
	- 변형력이 항복점 $S_y$ 이상으로 증가하면 막대는 영구히 변형된다.
	- 변형력이 계속 더 이상 증가하여 한계강도 $S_u$에 도달하면 막대는 결국 끊어진다.
...
**층밀리기**
$$\frac FA = G\cdot\frac{\Delta x}{L}$$
- $G$: 층밀리기 탄성률

**유압압축**
$$p = B\cdot\frac{\Delta V}{V}$$
- $B$: 부피탄성률

e.g. 12.05 늘어나는 막대의 변형력과 변형

e.g. 12.06 기우뚱거리는 책상 균형잡기
`silent camera 참고`
...
$$
\begin{align}
&\vec{F}_{1, net}  = \sum^n_{i=2}\vec{F}_{1i} \\
\approx& \vec{F}_1 = \int d\vec{F}
\end{align}
$$
...
$$
\begin{align}
& \frac{GMm}{R^2} - F_N = m\cdot a_{\text{centripital}} = m\cdot R \cdot \omega^2 \\
\rightarrow & \text{Weight} = F_N = m(a_g - a_{\text{centripital}})
\end{align}
$$
- 그러니까 결국은, 무게는 자전의 영향에 의해 중력과 완전히 같지는 않다는 이야기를 하고싶은 것이다.
...
$$F = \frac{GmM_{ins}}{r^2}$$
$$F = \frac{GmM}{R^3}r$$
- 중심에서 멀어질 수록 가속도가 커지는 효과가 난다.


## 13-5. 중력 퍼텐셜에너지

일단 공식부터 쓰면 다음과 같다.
거리 $r$만큼 떨어져 있는 질량이 $m$과 $M$인 두 물체의 중력 퍼텐셜에너지 $U$
$$U_g = -\frac{GMm}{r}$$

그럼 이제 증명을 해보자.
시작은 r이 무한, 즉 무한히 멀리 떨어져있는 경우로 시작해서
끝은 R만큼 떨어져있는 경우
$$
\begin{align}
\Delta U_g &= U_g(R) - U_g(\inf) = -\Delta W = -\int ^{final}_{initial}\vec{F}_g \cdot d\vec{r} \\
&= ... \\
&= -\frac{GMn}{R}
\end{align}
$$
Comment
- 값이 음수이며, r이 무한대로 가면 0으로 간다.
- Cannot divide it and say how much of it belongs to each object
- $M >> m$일 때 그냥 m의 중력 포텐셜에너지라 한다.

이제 해당 개념을 확장해보자.

e.g. 3개의 입자자
![200](../../Z.%20Docs/img/Pasted%20image%2020250421112105.png)
$$U_g = -\frac{Gm_1m_2}{r_{12}}-\frac{Gm_2m_3}{r_{23}}-\frac{Gm_3m_1}{r_{31}}$$
추가로 다음과 같은 성질들이 있다.
...
KE+GPE
- 고립계이며, Conservative 힘은 G-Force 밖에 없다.
$$K + U_g = \text{constant}$$
e.g. 지구 주위의 위성
e.g. 탈출속력 $v_{esc}$
- 이제는 에너지 보존법칙이 필요하다.
...
#### Kepler 법칙
> 행성의 운동을 지배하는 실험법칙으로, 후에 뉴턴이 Kepler의 법칙은 중력법칙을 따른다는 것을 증명하였다.

**궤도법칙**
```
모든 행성은 태양을 하나의 초점으로 하는 타원궤도로 운동한다.
```

plus alpha: 타원
![200](https://i.namu.wiki/i/7Cmfm2C7De2yRlxrOIEqdDL7mJb4HWF3GNBHe8SLjU0KPpO8qbySpMCANWE31OwLRo12va_6MbQwhtA46BAYRg.webp)
- 타원을 결정해주는 요소: Semi-major Axis, Semi-minor Axis
- Focus: $\text{Focus} = a \cdot e, \quad e \le 1$
- 이심률: $e = \sqrt{\frac{1-b^2}{a^2}}$
- e.g. 지구 궤도의 Exatricity e = 0.0167

**면적법칙**
```
행성과 태양을 연결하는 선분은 같은 시간 동안 같은 면적의 궤도면을 휩쓸고 지나간다.
```
즉 면적 $A$를 휩쓸고 지나가는 비율 $\frac{dA}{dt}$는 일정하다. `증명은 정리 pdf 참고`
$$\frac{dA}{dt} = \frac{L}{2m}$$
- $L$: 각운동량의 크기
- $m$: 질량

![450](../../Z.%20Docs/img/Pasted%20image%2020250430105244.png)

**주기법칙**
```
행성 주기의 제곱은 행성 궤도 장축의 세제곱에 비례한다.
```
...
- $G$: 
- $a(r)$: 궤도의 반지름
- e.g. $\frac{T^2}{a^3} = 3\times 10^{-34}\ yr^2\ m^{-3}$

![200](../../Z.%20Docs/img/Pasted%20image%2020250430105311.png)

## 13-7. 위성: 궤도와 에너지

원 궤도를 도는 위성에 대하여 `자세한 증명은 정리 pdf 참고`
$$E = K+U = -\frac{GMm}{2a}$$
...
아직까지도 정확히 밝혀진 건 없다.
하지만 부분적으로나마 증명된 것들을 한번 살펴보자.

**등가원리**
- 중력과 가속도가 동등하다. $a = g$
- 그러니까 그냥 서 있는 거랑 무중력 상태에서 가속도 $g$로 위로 운동하는 거랑 같다는 것이다.

선형적인 운동은 충분히 설명 가능하나 이것만으로는 불충분하다.
표면상으로 모든 것을 끌어당기는 지구의 중력은 어떻게 설명할 것인가?

**공간의 곡률**
- 중력은 휘어진 시공간에 의한 효과이다.

e.g. 중력 렌즈 효과
![200](https://upload.wikimedia.org/wikipedia/commons/1/11/A_Horseshoe_Einstein_Ring_from_Hubble.JPG)
- 아주 먼 천체에서 나온 빛이 중간에 있는 거대한 천체에 의해 휘어져 보이는 현상

**양자 중력**
- 중력자라는 입자에 의해 설명이 되어야 한다는 이론이다.

종합적으로 설명하자면, 아직 연구 중이다.
...
**유체**
- 담겨 있는 그릇의 형태에 따라 모양이 변하며, 흐를 수 있는 물질
- 유체를 특정짓는 요소: 밀도, 압력

**밀도**
$$\rho = \frac{m}{V}$$
- $(1\ \text{atm}) = 1.01 \times 10^5\ Pa = 760\ \text{torr}$
- 단위: $\text{kg/m}^3$

**압력**
$$p = \frac{F}{A}$$
- 평평한 면에 작용하는 균일한 힘에 의한 압력
- $F$: 면적 $A$에 작용하는 수직력의 크기
- 압력 $p$의 방향성은 논의할 필요가 없다.
- 단위: $\text{N/m}^2 = \text{Pa}$ `파스칼`

## 14-2. 정지해 있는 유체

![450](../../Z.%20Docs/img/Pasted%20image%2020250430112205.png)
- 물은 정지해 있으며, 작용하는 모든 힘은 평형을 이루고 있어서 정적 평형상태에 있다.

정수압: 정지한 유체 속에서 깊이 $h$에서의 압력 `증명은 정리 pdf 참고`
$$P = P_0 + \rho gh$$
- $P_0$: 대기압
- $P$: 절대압력
- $P - P_0$: 계기압력
...
e.g. pdf 374p. 확인문제 1
```
정적 평형상태인 유체 내 한 점에서의 압력은 깊이에만 의존하고 유체나 유체가 담겨 있는 그릇의 수평적 크기나 위치와는 상관이 없다.
```

## 14-3. 압력의 측정

e.g. 수은기압계, 대기압 측정
$$P_0 = \rho gh = 1.013\times 10^5\ \text{N/m}^2 \approx 1\ \text{atm}$$
- $\rho$: 수은의 밀도
...
밀폐된 용기에 담긴 비압축성 유체에 가해진 압력은 유체의 모든 지점에 같은 크기로 전달된다.

$$P = P_{\text{ext}} + \rho gh \rightarrow \Delta P_{\text{ext}} = \Delta P \quad \text{(for any h)}$$
Application, hydrolic lever
$$
\begin{align}
\Delta p_i &= \frac{F_i}{A_i} = P_i = P_0 \frac{F_0}{A_0} = \Delta p_0 \\
F_0 &= \frac{A_0}{A_i}\cdot F_i: \text{small } F_i \rightarrow \text{Large } F_0
\end{align}
$$
-> 작은 힘으로도 큰 힘을 만들어낼 수 있다. 그러나 대가가 있다. 밀어주는 거리가 그만큼 커져야 한다.

## 14-5. Archimedes의 원리

**Archimedes의 원리**

$$F_b = mf \cdot g = \rho v \cdot g$$
부력의 성질
- 방향: 위
- 크기: 물체에 의해서 대체된 유체의 무게

관련된 개념
- Equiliburium(floating)
$$
\begin{align}
F_g &= F_b \\
\rho_{text{body}}\ v_{\text{total}} \cdot g &= \rho_{\text{fluid}}\ v \cdot g
\end{align}
$$
- Apparent weight = Actual weight - 부력 = $F_g - F_b$

## 14-6 이상 유체의 운동

**이상적인 유체**

1. 유체의 속도가 어떤 한 지점을 고르면 상수이다.
2. 비압축성: 밀도가 상수이며, 균일하다.
3. 비점성 유체: 유체에 저항이 없다. `반례: 꿀`
4. 비회전성 유체: 어떤 지점을 중심으로 유체가 돌고 있는 경우를 생각하지 않는다.

**연속방정식**

![300](../../Z.%20Docs/img/Pasted%20image%2020250507111528.png)
...
![200](../../Z.%20Docs/img/Pasted%20image%2020250507111454.png)
$$p + \frac12 \rho v^2 + \rho gh = C$$
`증명은 교재 pdf 참고`

> 수평인 유선을 따라 유체요소가 움직이면서 속력이 빨라지면 유체의 압력은 감소해야 하고 그 역도 성립한다.
- 멈춰있는 유체에 대해서: $p_1 + \rho gh_1 = p_2 + \rho gh_2$ 즉, Bernoulli 방정식이 더 일반화된 방정식이다.
- $h_1 = h_2$ 이면, 유체의 속도가 빠를 수록 압력이 작아진다.

e.g. 확인문제 4
- 연속방정식과 Bernoulli 방정식을 이용하여 속력과 압력을 각각 비교해보자.
- 속력: $Av = C$ 이용
$$v_1 > v_2 = v_3 > v_4$$
- 압력:
$$p_1 < p_2 < p_3 < p_4$$

`5/14 지난 시간에 한 내용 연습문제 많이 풀어보라고 함`
